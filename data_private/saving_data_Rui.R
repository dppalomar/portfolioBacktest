...
N <- ncol(stock_SP500_2008_2018.RData)
T <- nrow(stock_SP500_2008_2018.RData)
for(i in 1:30) {
  t_start <- sample(T-T_sample+1, 1)
  mask <- !is.na(stock_SP500_2008_2018.RData[t_start, ])
  mask <- rep(1:N)[mask]
  if (length(mask) <= N_sample) {
    stock_mask <- mask
  } else {
  ...
  prices <- c(prices, list(stock_SP500_2008_2018.RData[t_start:(t_start+T_sample-1), stock_mask]))
}
