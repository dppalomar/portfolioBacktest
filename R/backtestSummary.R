...
#' @param portfolio_names Names of portfolios to select (by default all).
#' @param portfolio_indexs Indexes of a portfolio.
#' @param summary_funs Summary function to be employed (e.g., \code{median} or \code{mean}).
#' @param show_benchmark Logical value indicating whether to include benchmark in the summary.
#' 
#' @return List with the following elements:
#' \item{\code{performance_summary}}{performance criteria like expected return, volatility, 
#'                                   Sharpe ratio, drawdown, turnover rate, return 
#'                                   on investment, computational time, etc.}
...
#' \item{\code{error_message}}{Error messages generated by each portfolio function. 
...
#' 
#' @export
backtestSummary <- function(bt, portfolio_names = NA, portfolio_indexs = NA,
                            summary_fun = median, show_benchmark = TRUE) {
  if (anyNA(portfolio_names) && anyNA(portfolio_indexs)) portfolio_indexs <- attr(bt, 'portfolio_index')
  if (!anyNA(portfolio_indexs)) portfolio_names <- names(bt)[portfolio_indexs]
...
#' 
#' @export  
backtestTable <- function(bt, portfolio_names = NA, portfolio_indexs = NA, 
...
#' 
#' @param bt Backtest results as produced by function \code{\link{portfolioBacktest}}.
#' @param portfolio_name Name of a single portfolio.
#' @param portfolio_index Index of a portfolio.
#' @param selector Vector of strings to choose performance criteria (by default all).
#' 
#' @return List of desired results.
#' 
...
#' @export
#' 
backtestSelector <- function(bt, portfolio_name = NA, portfolio_index = NA, selector = NULL) {
