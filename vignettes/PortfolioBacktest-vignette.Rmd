...
Backtesting multiple portfolios is equally simple. It suffices to pass a list of functions to the backtesting function `multiplePortfolioBacktest()` [to be completed]:
```{r}
res <- multiplePortfolioBacktest(portfolio_fun_list = list(uniform_portfolio_fun, GMVP_portfolio_fun), prices =  prices[1:5], shortselling = TRUE)
res
```
...
```{r, cache=TRUE}
res_all_students <- multiplePortfolioBacktest(folder_path = "folder_path", prices =  prices[1:3])
res_all_students$performance_summary
res_all_students$cpu_time_average
res_all_students$failure_ratio
```

Now we can rank the different portfolios/students based on a weighted combination of the performance measures [to be completed]:
```{r, eval=FALSE}
final_ranking <- portfolioRank(res_all_students)
final_ranking
```
