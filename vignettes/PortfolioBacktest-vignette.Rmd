...
The results from backtesting on multiple datasets can be further summarized by function `backtestSummary()` based on user customized summary functions. For example, we can summarize results using `median()` and `mean()`
```{r, cache=TRUE}
...
names(bt)
res_summary <- backtestSummary(bt, summary_funs = list('median' = median))
res_summary$performance_summary_median
```
...
res_all_students <- backtestSummary(bt_all_students, summary_funs = list('median' = median))
