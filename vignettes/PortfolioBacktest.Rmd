...
Note that an unexpected error might be thrown out when run parallel backtesting through [RStudio in macOS](https://github.com/rstudio/rstudio/issues/6692). If that happens, one can check the default parallel setting via:
```{r, eval=FALSE}
parallel:::getClusterOption("setup_strategy")
```

If `"parallel"` is returned, one can set the option `setup_strategy` be `"sequential"` via:
```{r, eval=FALSE}
