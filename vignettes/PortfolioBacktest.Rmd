...
## Basic structure of `dataset_list`
...
head(dataset10$`dataset 1`$adjusted[,1:3], 5)  
```
...
my_dataset_list <- stockDataResample(SP500_YAHOO, N_stock = 50, T_sample = 252*2, N_dataset = 10)
```
...
backtestSelector(bt, portfolio_name = "Quintile", selector = c("Sharpe ratio", "max drawdown"))
```
...
## Backtest over files: usage for grading students
...
We are going to add the following features into our package:
