...
over randomized multiple datasets on a rolling-window basis, as described in Chapter 8 (Portfolio Backtesting) of [@Palomar2025].
It can be used by researchers and practitioners to backtest a set
of different portfolios, as well as course instructors to assess the students in their portfolio design
in a fully automated and convenient manner. The results can be nicely formatted in tables and plots.


# Package Snapshot
Backtesting is a dangerous task fraught with many potential pitfalls [@Palomar2025; @Luo_etal2014_sevensins]. By performing a large number of randomized backtests, instead of visually inspecting a single backtest, one can obtain more realistic results.
...
(dtable <- summaryTable(res_sum, type = "DT", order_col = "Sharpe ratio", order_dir = "desc"))

# # to save table to png file
# library(htmlwidgets)
# library(webshot)
# html_file <- "figures/dtable.html"
# saveWidget(dtable, html_file)
# webshot(html_file, "figures/dtableSnapshot.png") # you can also export to pdf
```
...
```{r backtesting-portfolios, results="hide"}
...
SP500 <- stockDataDownload(stock_symbols = SP500_symbols, 
                           from = "2008-12-01", to = "2018-12-01",
                           local_file_path = file.path(getwd(), "figures"))
...
```{r stacked-bar, warning=FALSE, fig.width=7, fig.height=3}
...
```{r transaction-cost, message=FALSE}
library(ggfortify)
...
```{r parallel-backtesting, cache=TRUE}
...
- **Sterling ratio**: the return over average drawdown, see [here](https://en.wikipedia.org/wiki/Sterling_ratio) for complete definition. In the package, we use $$
 \text{Sterling ratio} = \frac{\text{annualized return}}{\text{max drawdown}};$$
- **Omega ratio**: the probability weighted ratio of gains over losses for some threshold return target, see [here](https://en.wikipedia.org/wiki/Omega_ratio) for complete definition. The ratio is calculated as: $$ \Omega(r) = \frac{\int_{r}^{\infty} (1-F(x))dx}{\int_{-\infty}^{r} F(x)dx};$$
...
A detailed treatment of portfolio performance measures is given in [@Palomar2025, Ch. 6]. One can easily add new performance measures with the function `add_performance()`.
