...
* **Step 3** - do backtest
```{r, results = 'hide'}
bt <- portfolioBacktest(my_portfolio, dataset)
```
* **Step 4** - check your portfolio performance
```{r}
backtestSelector(bt, 1)$performance
```

# Loading Date

## Basic Structure of Dataset
The main function `portfolioBacktest()` requires argument `dataset` follow the certain format: it should be a collection (list) of several individual data items, with each of them be a list of several xts objects following exactly same date index. Besides, each item must contain an xts recording the historical prices of all stocks. We have embedded a small formated dataset in this package, see follows
```{r}
data("dataset")  # load the embedded dataset
class(dataset)  # show dataset class
names(dataset)  # show names of (10) individual data items
str(dataset$`dataset 1`)  # show structure of one data item
```
...
## Obtain More Data
We notice that 10 data is not enough in evaluating or comparing your portfolio functions. In this package, we provide a function `stockDataDownload()` to download online data resources. Its results will naturally follow our required data format. Function `stockDataResample()` can help do resampling from on a formated data and return a formated dataset, which can be directly passed to `portfolioBacktest()`. We recommend you to use these two functions in generating your own dataset, e.g.,
```{r, eval = FALSE}
data("SP500_symbols")  # load the SP500 symbols
# download data from internet
SP500 <- stockDataDownload(stock_symbols = SP500_symbols, from = "2008-12-01", to = "2018-12-01")
# resample from SP500, each with 50 stcoks and 2-year continuous data 
my_dataset <- stockDataResample(SP500_YAHOO, N_stock = 50, T_sample = 252*2, N_dataset = 10)
```
In each individual data item, you will get the 7 xts objects with name: `open`, `high`, `low`, `close`,   `volumn`, `adjusted`, `index`. You can save the dataset and use it everywhere as you want.
...
for (i in 1:length(dataset))
  dataset[[i]]$MACD <- apply(dataset[[i]]$adjusted, 2, function(x){TTR::MACD(x)[ ,'macd']})
```
...
bt <- portfolioBacktest(portfolios, dataset)
```
...
Feel free to check your backtest results by directly visiting returned `bt`. 

## Make Your Results In Order

We also provide a serial of functions to conveniently extract results:

* Select several porformances of one specific portfolio
```{r}
# select sharpe ratio and max drawdown performance of uniform portfolio
backtestSelector(bt, "Uniform", selector = c("Sharpe ratio", "max drawdown"))
```
* Tables of several porformances of several portfolios (classified by performance criterias)
```{r}
# show the portfolios performance in tables 
backtestTable(bt, c("Uniform", "GMVP", "Markowitz"), selector = c("Sharpe ratio", "max drawdown"))
```
* Summary the performances
```{r}
...
## View Your Results In Figures

Beside, we also provide some functions to show results in figures:

* Tables
```{r}
summaryTable(res_sum, measures = c("cpu time", "Sharpe ratio", "max drawdown"), type = "grid.table")
```
* Barplot
```{r}
summaryBarPlot(res_sum)
```
* BoxPlot
```{r}
backtestBoxPlot(bt)
...
```{r, cache=TRUE}
bt <- portfolioBacktest(portfolios, dataset, benchmark = c('uniform', 'index'))
names(bt)
```
...
```{r, eval = FALSE}
bt <- portfolioBacktest(portfolios, dataset, show_progress_bar = TRUE)
```
...
system.time(bt_pardata <- portfolioBacktest(Markowitz_portfolio_fun, dataset, paral_datasets = 5))
```
...
## Backtest Over Files: Usage for Grading Students in A Course
...
```{r, cache=TRUE}
bt_all_students <- portfolioBacktest(folder_path = "portfolio_files", dataset = dataset)
names(bt_all_students)
```

## Rank Portfolios Performance By Your Rule
...
leaderboard <- portfolioLeaderboard(bt_all_students, weights = list('Sharpe ratio' = 7, 
                                                                    'max drawdown' = 1,  
                                                                    'annual return' = 1, 
                                                                    'ROT bps' = 1))
